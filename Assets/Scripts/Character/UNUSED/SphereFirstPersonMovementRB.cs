...
public class SphereFirstPersonMovementRB : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        inAirLerpSpeed = defaultMoveSpeed / 550.0f;
    ...
    private void PlayerMovementHelper()
    {
        ...
        gameObject.transform.rotation = Quaternion.FromToRotation(gameObject.transform.up, Vector3.zero - gameObject.transform.position) * gameObject.transform.rotation;
...
/*        if (Physics.Raycast(gameObject.transform.position + inputDirection.normalized * 0.25f, -gameObject.transform.up, out RaycastHit hit, Mathf.Infinity, playerMask, QueryTriggerInteraction.Ignore) 
       ...
       */
        if (Physics.Raycast(gameObject.transform.position, -gameObject.transform.up, out RaycastHit hit, Mathf.Infinity, playerMask, QueryTriggerInteraction.Ignore))
        {
            groundNormal = hit.normal;
        }
    }
    ...
    private void PlayerJump()
    {
        if (Input.GetKeyDown(KeyCode.Space) && IsGrounded())
        {
                ...
                playerRB.AddForce(gameObject.transform.up * jumpScalar, ForceMode.VelocityChange);
        }
    ...
    private void PlayerGravity()
    {
        if (!IsGrounded())
        {
            playerRB.AddForce(-gameObject.transform.up * gravityScalar, ForceMode.Acceleration);
            /*
    ...
    {
        if (Physics.SphereCast(gameObject.transform.position, 0.49f, -gameObject.transform.up, out _, 0.4344f, playerMask, QueryTriggerInteraction.Ignore))
    ...
    {
        if (inputVect.magnitude != 0 || Input.GetKeyDown(KeyCode.Space))
        {
            return true;
        }
        else return false;
